...
function boolish(val) {
	...
	if (val == null)
		return false;
	return lc(trim(`${val}`)) === 'true';
};

function safe_token(val, pattern) {
	if (val == null)
		return null;
	const s = trim(`${val}`);
	if (!s || s === '')
		return null;
	return match(s, pattern) ? s : null;
};

function get_cli_host_arg() {
	const ctx = cursor();
	let host = ctx.get_first('dockerd', 'globals', 'hosts')?.[0] || null;
	ctx.unload();

	host = safe_token(host, /^[A-Za-z0-9._:\/\[\]-]+$/);
	if (!host)
		return null;

	// docker CLI expects docker-style schemes
	if (index(host, 'http://') === 0)
		host = 'tcp://' + substr(host, 7);
	else if (index(host, 'https://') === 0)
		host = 'tcp://' + substr(host, 8);
	else if (index(host, '://') === -1 && index(host, '/') === 0)
		host = 'unix://' + host;
...
/*
`/containers/{id}/logs` may return multiplexed binary stream frames
(`application/vnd.docker.multiplexed-stream`) which include NUL bytes.
Those bytes are not reliable through JSON-RPC transport, causing truncation
like body == "\u0002". Use docker CLI to get already demuxed text output.
*/
function call_docker_logs_text(request) {
	const args = request?.args || {};
	const q = args?.query || {};
	const id = safe_token(args?.id, /^[A-Za-z0-9_.:-]+$/);

	if (!id) {
		return {
			code: 400,
			headers: {},
			body: { message: 'Invalid container id' }
		};
	}

	const host = get_cli_host_arg();
	let cmd = 'docker';
	if (host)
		cmd += ` -H ${host}`;
	...
	const tail = safe_token(q?.tail, /^[0-9]+$/) || '100';
	cmd += ` --tail ${tail}`;

	if (boolish(q?.timestamps))
		cmd += ' --timestamps';
	if (boolish(q?.follow))
		cmd += ' --follow';

	const since = safe_token(q?.since, /^[A-Za-z0-9TZ:_.+\-]+$/);
	if (since)
		cmd += ` --since ${since}`;

	const until = safe_token(q?.until, /^[A-Za-z0-9TZ:_.+\-]+$/);
	if (until)
		cmd += ` --until ${until}`;

	cmd += ` ${id}`;

	const tmpfile = `/tmp/.dockerman_logs_${time()}_${id}`;
	const shell_cmd = `${cmd} > ${tmpfile} 2>&1`;
	const rc = system(shell_cmd);
	const output = fs.readfile(tmpfile) || '';
	system(`rm -f ${tmpfile}`);
	const ok = (rc === 0 || rc === true);

	if (!ok) {
		return {
			code: 500,
			headers: { 'content-type': 'application/json' },
			body: { message: trim(output) || 'docker logs failed' }
		};
	}
...
const container_methods = {
	...
	logs:    { args: { id: '', query: {} }, call: (request) => call_docker_logs_text(request) },
